...
import { Button, Card, EmptyState, Modal, Input, Textarea, Badge, useToast } from "@vrm/ui";
    ...
    <>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-primary-400">Get help with your bookings and account.</p>
        </div>
      ...
      {!isLoading && !data?.data.length ? (
        ...
        <div className="flex flex-col gap-3">
          {(data?.data ?? []).map((ticket) => (
            <Link key={ticket.id} to={`/account/support/${ticket.id}`}>
