...
class _SupportTicketDetailPageState extends ConsumerState<SupportTicketDetailPage> {
  ...
  Future<void> _updateStatus(String status) async {
    await ref.read(marketplaceApiProvider).support.updateStatus(widget.ticketId, status);
    ref.invalidate(_ticketProvider(widget.ticketId));
  }
          ...
          children: [
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(ticket.user?.fullName ?? "", style: TextStyle(color: AppColors.mutedTextOf(context))),
                  ...
                  Wrap(
                    ...
                    children: [
                      for (final (status, label) in const [
                        ...
                        if (ticket.status.toJson() != status)
                          OutlinedButton(onPressed: () => _updateStatus(status), child: Text(label)),
                    ],
                  ),
                ],
              ),
                ...
                itemBuilder: (context, i) {
                  ...
                  return Align(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(color: isMine ? AppColors.secondary : AppColors.subtleFillOf(context), borderRadius: BorderRadius.circular(14)),
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(message.message, style: TextStyle(color: isMine ? Colors.white : AppColors.textOf(context))),
                          const SizedBox(height: 4),
                          Text(formatDateTime(message.createdAt), style: TextStyle(fontSize: 10, color: isMine ? Colors.white70 : AppColors.mutedTextOf(context))),
                        ],
                      ),
                    ),
                  );
            ...
            SafeArea(
              child: Padding(
                ...
                child: Row(
                  children: [
                    Expanded(child: TextField(controller: _message, decoration: const InputDecoration(hintText: "Reply…"))),
